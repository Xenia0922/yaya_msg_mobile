...
test('无名 hint 字段不误接受（避免时间戳/干扰字段误读）', () => {
  // fix5 起已删除「未知字段兜底遍历」：字段名不在白名单（unknown/answer/time/seconds/length）一律不接受
  assert.equal(findMediaDurationSeconds({ unknown: 183000 }), 0);
  assert.equal(findMediaDurationSeconds({ answer: 5000000 }), 0);
  assert.equal(findMediaDurationSeconds({ time: 1724123456 }), 0);
});

test('白名单毫秒字段大值正常归一（明确 Ms 后缀，> 2h 也接受）', () => {
  assert.equal(findMediaDurationSeconds({ durationMs: 183000 }), 183); // 3min
  assert.equal(findMediaDurationSeconds({ videoTimeMs: 5000000 }), 5000); // 1h23min
});
