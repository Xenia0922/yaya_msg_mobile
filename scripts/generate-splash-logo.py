...
- launcher legacy 图标 ic_launcher.png / ic_launcher_round.png：frac=1.0（铺满），
  圆角外填充浅色 #f0f0f0（与关于页 logo backgroundColor 一致），桌面即为「圆角 logo 浅底」= 关于页同款。
...
LIGHT_TILE = (240, 240, 240, 255)  # #f0f0f0 与关于页 logo backgroundColor 一致
...
def main() -> None:
    ...
    # launcher legacy：铺满，圆角外浅底（= 关于页）
    for name in ("ic_launcher.png", "ic_launcher_round.png"):
        for path in sorted(glob.glob(os.path.join(ROOT, "android/app/src/main/res/mipmap-*", name))):
            size = Image.open(path).size[0]
            out = make_rounded(src, size, 1.0, tile=LIGHT_TILE)
            out.save(path)
            print(f"wrote {os.path.relpath(path, ROOT)} {size}x{size} frac=1.0 light-tile r={int(round(size * RADIUS_RATIO))}")
    # foreground 保留（若启用自适应时同款），同样铺满浅底
    for path in sorted(glob.glob(os.path.join(ROOT, "android/app/src/main/res/mipmap-*", "ic_launcher_foreground.png"))):
        size = Image.open(path).size[0]
        out = make_rounded(src, size, 1.0, tile=LIGHT_TILE)
        out.save(path)
        print(f"wrote {os.path.relpath(path, ROOT)} {size}x{size} fg light-tile r={int(round(size * RADIUS_RATIO))}")
